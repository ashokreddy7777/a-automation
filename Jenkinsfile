pipeline{
  ...
  stages{
    ...
    stage('s3 download'){
      steps{
        withAWS(credentials: 'aws'){
          s3Download (pathStyleAccessEnabled: 'true', bucket: 'a-automation', file: 'apps/FRIENDS9-0.0.1-SNAPSHOT.war', path: '/home/ak/jenkins_home/workspace/a-automation_CI_feature_ashok/target/', force: 'true')
        }
      }
    }
